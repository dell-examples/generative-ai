...
class Reciever:
    def __init__(self, key, broker, udp_port, logger):
        ...
        self.ffmpeg_process = (
            ...
            .output(f"udp://{self.IPAddr}:{udp_port}", f="mpegts")
            # .output(f"udp://localhost:{udp_port}", f="mpegts", vcodec="libx264", g=2)
            # .output(f"rtp://localhost:{udp_port}?pkt_size=1316", format="h264", vcodec="libx264", g=5)
            .overwrite_output()
