				...
				for _, value := range streams_map {
					...
					if err != nil {
						...
						if stream_map["visualize"] == true {
							udpPort := initialPort + i
							fmt.Println(udpPort)
							pipeline := fmt.Sprintf("udpsrc port=%v buffer-size=524288 ! application/x-rtp,media=video,clock-rate=90000,encoding-name=H264 ! rtph264depay ! h264parse ! queue ! avdec_h264 ! videoconvert ! queue", udpPort)
