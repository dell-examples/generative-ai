...
function getBotResponse(text) {
  botResponse = `<img class="botAvatar" src="./static/img/scalersai-logo.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
  return botResponse;
}
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const BotResponse = `<img class="botAvatar" src="./static/img/scalersai-logo.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            ...
            if (
              ...
              html = html.replaceAll("<br>", "");
              // botResponse = `<img class="botAvatar" src="./static/img/scalersai-logo.png"/><span class="botMsg">${html}</span><div class="clearfix"></div>`;
              botResponse = getBotResponse(html);
            } else {
              // if no markdown formatting found, render the text as it is.
              if (!botResponse) {
                botResponse = `<img class="botAvatar" src="./static/img/scalersai-logo.png"/><p class="botMsg">${html}</p><div class="clearfix"></div>`;
              }
            }
...
function actionTrigger() {
  $.ajax({
    url: `http://${ip_address}:5005/conversations/${sender_id}/execute`,
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
      name: action_name,
...
function customActionTrigger() {
  $.ajax({
    url: `http://${ip_address}:5055/webhook/`,
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
