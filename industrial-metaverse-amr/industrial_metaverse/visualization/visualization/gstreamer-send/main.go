...
func main() {
	...
	conf_file, err := ioutil.ReadFile("/config/metaverse_config.yaml")
				...
				for _, value := range streams_map {
					...
					if err != nil {
						...
						if stream_map["visualize"] == true {
							...
							if single == true {
								pipeline = fmt.Sprintf("udpsrc port=%v buffer-size=524288 ! application/x-rtp,media=video,clock-rate=90000,encoding-name=H264 ! rtph264depay ! h264parse ! queue ! avdec_h264 ! videoconvert ! queue", stream_map["udp_port"])
							} else {
								pipeline = fmt.Sprintf("rtspsrc location=rtsp://localhost:8554/%v ! decodebin ! videoconvert ! queue", stream_map["udp_port"])
							}
